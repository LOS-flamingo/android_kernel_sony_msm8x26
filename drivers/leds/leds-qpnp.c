...
/*
 * Keep blinking enabled on Flamingo for non-manual PWM modes,
 * as it uses custom LUT-based LED control
 */
...
#ifdef CONFIG_SONY_FLAMINGO
			/*
			 * Reset the LUT pause counters to 0 when the LED is off
			 * to prevent any residual blinking or hold in the PWM LUT.
			 */
			pwm_cfg->lut_params.lut_pause_hi = 0;
			pwm_cfg->lut_params.lut_pause_lo = 0;
#endif
...
#ifdef CONFIG_SONY_FLAMINGO
	/*
	 * For Flamingo MPP LED, PWM LUT handles blinking;
	 * set brightness directly only when turning off.
	 * Other LEDs follow standard on/off behavior.
	 */
	if(led->id == QPNP_ID_LED_MPP)
	{
		if(blinking == 0)
...
#ifdef CONFIG_SONY_FLAMINGO
		/*
		 * For Flamingo, ensure LEDs are fully turned off on reboot.
		 * MPP LED requires explicit qpnp_led_turn_off to disable PWM LUT.
		 */
		{
			led->cdev.brightness = LED_OFF;
			if (led->id == QPNP_ID_LED_MPP)
